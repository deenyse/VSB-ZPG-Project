...
RandomTranslation::RandomTranslation(float range, float cubeSize, float speed)
    : Translate(glm::vec3(0.0f)), range(range), halfCubeSize(cubeSize / 2.0f), speed(speed)
{
...
void RandomTranslation::update(float deltaTime) {
    ...
    // перемещаем объект по ломаной линии с учётом скорости
    t += speed * deltaTime;
    ...
    if (t >= 1.0f) {
        t = 0.0f;
        currentIndex++;
        controlPoints.push_back(randomPointInCube()); // новая точка для следующего сегмента
    }
}
