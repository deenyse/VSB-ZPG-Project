...
void App::run() {
	while (!glfwWindowShouldClose(window)) {
		float deltaTime = inputManager->getDeltaTime();
		glfwPollEvents();

		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);

		sceneManager->updateCameraPosition(inputManager->getMoveDirection(), inputManager->getAndResetMouseOffset(), deltaTime);

		sceneManager->renderCurrentScene(deltaTime);
